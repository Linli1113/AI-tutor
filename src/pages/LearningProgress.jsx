...
function LearningProgress({ user, updateUser }) {
  ...
  const wrongCountInAnswered = totalWrong
  const accuracy = totalQuestions > 0 ? Math.round((correctQuestions / totalQuestions) * 100) : 0

  // 获取错题本中的完整题目信息
  const wrongBookQuestions = wrongBookIds.map(id => {
    const q = allQuestionsList.find(question => question.id === id)
    if (q) {
      return { ...q, answerData: answeredQuestions[id] }
    }
    return null
  }).filter(Boolean)
  ...
  const loadUserData = () => {
    ...
    if (currentUser) {
      return {
        ...
        wrongBookIds: currentUser.wrongBookIds || [],
  ...
  // 渲染练习历史题目
  const renderPracticeQuestion = (item) => {
    const question = allQuestionsList.find(q => q.question === item.question)
    if (!question) {
      return (
        <div key={item.id} className="history-item">
          <div className="history-question">
            <span style={{ color: '#667eea', marginRight: '8px' }}>[{item.type}]</span>
            {item.question}
          </div>
          <div className="history-meta">
            <span>知识点: {item.knowledge}</span>
            <span>做题时间: {item.time}</span>
    ...
    return (
      <div key={item.id} className="history-item">
        ...
        {item.type === '选择题' && question.options && (
          <div className="options-display">
            <div className="options-label">选项：</div>
            {question.options.map((opt, optIndex) => {
              const isCorrectOption = question.answer === optIndex
              const isUserAnswer = typeof item.userAnswer === 'number' ? 
                optIndex === item.userAnswer : 
                opt === item.userAnswer
              return (
                <div 
                  key={optIndex} 
                  className={`option-row ${isCorrectOption ? 'correct' : isUserAnswer ? 'wrong' : ''}`}
                >
                  {String.fromCharCode(65 + optIndex)}. {opt}
                  {isCorrectOption && <span style={{ color: '#10b981', marginLeft: '8px' }}> ✓ 正确</span>}
                  {isUserAnswer && !isCorrectOption && <span style={{ color: '#ef4444', marginLeft: '8px' }}> ✗ 你的答案</span>}
                </div>
              )
            })}
          </div>
        ...
        {!item.correct && (
          <div className="answer-detail">
            你的答案: {item.userAnswer} | 正确答案: {item.type === '选择题' ? question.options[question.answer] : item.correctAnswer}
          </div>
        )}
      </div>
    )
  ...
  // 渲染错题本题目
  const renderWrongBookQuestion = (item) => {
    const isCorrect = item.answerData?.correct
    const isInWrongBook = userData.wrongBookIds.includes(item.id)
    return (
      <div key={item.id} className="history-item wrong-book-item">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
          <div className="history-question">
            <span style={{ color: '#667eea', marginRight: '8px' }}>[{item.type || item.answerData?.questionType || '题目'}]</span>
            {item.question}
          </div>
          <button
            onClick={() => toggleWrongBook(item.id)}
            style={{
              background: 'none',
              border: 'none',
              cursor: 'pointer',
              fontSize: '20px',
              color: isInWrongBook ? '#ef4444' : '#9ca3af'
            }}
            title={isInWrongBook ? '移除错题本' : '加入错题本'}
          >
            {isInWrongBook ? '★' : '☆'}
          </button>
        </div>
        ...
        {item.options && (
          <div className="options-display">
            ...
            {item.options.map((opt, optIndex) => {
              const isCorrectOption = item.answer === optIndex
              const isUserAnswer = item.answerData?.userAnswer === opt || 
                (typeof item.answerData?.userAnswer === 'number' && item.answerData?.userAnswer === optIndex)
              return (
                <div 
                  key={optIndex} 
                  className={`option-row ${isCorrectOption ? 'correct' : isUserAnswer ? 'wrong' : ''}`}
        ...
        <div className="answer-detail">
          你的答案: {item.answerData?.userAnswer} | 正确答案: {item.options ? item.options[item.answer] : item.answer}
        </div>
      </div>
    )
  }
    ...
    <div style={{ position: 'relative', minHeight: '100vh' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 10 }}>
        <div className="stats-grid">
          <div className="stat-card" style={{ background: 'linear-gradient(135deg, #FFF3D6 0%, #FFE8A6 100%)', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.06)' }}>
            <div className="stat-value">{totalQuestions}</div>
            <div className="stat-label">总做题数</div>
          </div>
          <div className="stat-card" style={{ background: 'linear-gradient(135deg, #FFD5E5 0%, #FFC5D5 100%)', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.06)' }}>
            <div className="stat-value">{correctQuestions}</div>
            <div className="stat-label">正确题数</div>
          </div>
          <div className="stat-card" style={{ background: 'linear-gradient(135deg, #E8F5FF 0%, #D5EEFF 100%)', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.06)' }}>
            <div className="stat-value">{accuracy}%</div>
            <div className="stat-label">正确率</div>
          </div>
          <div className="stat-card" style={{ background: 'linear-gradient(135deg, #E5FFE5 0%, #D5FFD5 100%)', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.06)' }}>
            <div className="stat-value">{wrongCountInAnswered}</div>
            <div className="stat-label">错题数</div>
          </div>
        </div>

        <div className="card wrong-book-card" style={{ background: 'linear-gradient(135deg, #FFF3D6 0%, #FFE8A6 100%)', borderRadius: '24px', padding: '28px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.06)' }}>
          <h2>错题本 📝 <span style={{ fontSize: '14px', color: '#8B6B4D', fontWeight: 'normal' }}>({wrongBookQuestions.length}题)</span></h2>
          {wrongBookQuestions.length > 0 ? (
            <div className="history-list">
              {wrongBookQuestions.map(renderWrongBookQuestion)}
            </div>
          ) : (
            <div className="text-center py-8" style={{ color: '#8B6B4D' }}>
              还没有错题，继续保持！ 🎉
            </div>
          )}
        </div>
        ...
        <div className="card" style={{ background: 'linear-gradient(135deg, #FFD5E5 0%, #FFC5D5 100%)', borderRadius: '24px', padding: '28px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.06)' }}>
          ...
          {practiceHistory.length > 0 ? (
            <div className="history-list">
              {practiceHistory.slice(0, 20).map(renderPracticeQuestion)}
            </div>
          ) : (
            <div className="text-center py-8" style={{ color: '#8B6B4D' }}>
              还没有练习记录，快去做题吧！
